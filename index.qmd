...
$$
y_t=\left(\begin{array}{cc}\operatorname{pce}_t & =\text { PCE } \\\operatorname{GT}_t & =\text { Google Trends index } \\\operatorname{inc}_t & =\text { Real household disposable income } \\\text { int }_t & =\text { Interest rates on three-month treasury bills} \\\text { sent }_t & =\text { Consumer sentiment indicator } 
\end{array}\right)
$$
...
In the continuation of this research proposal, we will make assumptions on the priors distribution and a possible extension of this model could be to check if other priors could lead to better results. Moreover, we will estimate the matrix A to understand the relationship between those variables. 
